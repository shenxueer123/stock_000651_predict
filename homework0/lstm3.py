...
# 获取训练集
def get_train_data(batch_size=6, time_step=2, train_begin=0, train_end=500):
    batch_index = []
...
def get_test_data(time_step=20, test_begin=500):
    ...
    mean = np.mean(data_test, axis=0)#一共有5个特征，按列求的
...
def train_lstm(batch_size=6, time_step=2, train_begin=0, train_end=500):
